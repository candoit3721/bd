-- Add contact fields to party_settings table
DO $$
BEGIN
  -- Add contact_email column if it doesn't exist
  IF NOT EXISTS (
    SELECT FROM information_schema.columns
    WHERE table_name = 'party_settings' AND column_name = 'contact_email'
  ) THEN
    ALTER TABLE party_settings ADD COLUMN contact_email TEXT;
  END IF;

  -- Add contact_phone column if it doesn't exist
  IF NOT EXISTS (
    SELECT FROM information_schema.columns
    WHERE table_name = 'party_settings' AND column_name = 'contact_phone'
  ) THEN
    ALTER TABLE party_settings ADD COLUMN contact_phone TEXT;
  END IF;
  ...
  -- Add secondary_contact_name column if it doesn't exist
  IF NOT EXISTS (
    SELECT FROM information_schema.columns
